...
def build_prompt(text_content, rec_texts):
...
def format_number_preserve_zeros(value):
...
def apply_number_formatting(invoice):
...
def _normalize_tva(raw):
    """Normalize OCR TVA text without forcing it to predefined rates."""
...
def normalize_invoice_data(data):
    ...
    invoice = dict(data) if isinstance(data, dict) else {}
...
def build_current_invoice(edited_lines, session_state):
...
def seed_editor_state(invoice, session_state):
...
def invoice_to_excel_bytes(invoice):
